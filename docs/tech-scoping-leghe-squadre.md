...
- [x] **Step 4** — Admin: `/admin/teams`, `/admin/leagues`, `requireLeagueRole` in `dal.ts` *(implementato il 2026-09-10 — CRUD minimo su Team, creazione lega con OWNER automatico, gestione membri con guardia "ultimo OWNER", testato end-to-end su staging via Playwright. `admin/teams/[teamId]/matches` (sostituzione di `admin/matches/page.tsx`) rimandato: valore nullo con una sola squadra, da fare quando arriva la seconda — vedi §7 punto 7. Corretto anche un bug di scoping team pre-esistente segnalato nel doc: la rosa marcatore in `admin/matches/[id]/result` e nel form pronostico giocatore non filtrava per `teamId`)*
